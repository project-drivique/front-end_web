...
export default function Verificar2FAPage() {
  ...
  const c = {
    ...
    panelLeftBg: esModoOscuro ? 'linear-gradient(160deg, #070d1e 0%, #0f172a 55%, #1e293b 100%)' : 'linear-gradient(140deg, #071b4d 0%, #123a93 55%, var(--brand-primary) 100%)',
    ...
    cardShadow: esModoOscuro ? '0 22px 60px rgba(0,0,0,0.5)' : '0 22px 60px rgba(var(--brand-primary-rgb), 0.14)',
    ...
    inputBorder: esModoOscuro ? '#23314d' : 'var(--brand-soft-strong-light)',
    ...
    cancelBg: esModoOscuro ? '#172136' : 'var(--brand-soft-light)',
    cancelBorder: esModoOscuro ? '#23314d' : 'var(--brand-border-light)',
    cancelText: esModoOscuro ? 'var(--brand-accent)' : 'var(--brand-primary)',
  }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(circle at top left, rgba(255,255,255,0.18), transparent 34%), radial-gradient(circle at bottom right, rgba(var(--brand-accent-rgb),0.16), transparent 32%)' }} />
          ...
          <div style={{ position: 'relative', zIndex: 1, display: 'flex', alignItems: 'center', gap: '10px' }}>
            <img src={brand.logoDataUrl || logo} alt={brand.name} style={{ height: '40px', width: 'auto', display: 'block', filter: 'drop-shadow(0 4px 12px rgba(0,0,0,0.25))' }} />
            <span style={{ fontFamily: 'Outfit, Inter, sans-serif', fontWeight: 900, fontSize: '18px', letterSpacing: '0.14em', color: 'var(--brand-text-dark)', textTransform: 'uppercase' }}>
              {brand.name}
            </span>
          </div>

          <div style={{ position: 'relative', zIndex: 1, maxWidth: '340px' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '8px 14px', borderRadius: '999px', background: 'rgba(255,255,255,0.14)', color: 'var(--brand-soft-strong-light)', fontSize: '13px', fontWeight: 700, marginBottom: '16px', border: '1px solid rgba(255,255,255,0.18)' }}>
              <FaShieldAlt size={14} color="var(--brand-accent)" />
          ...
          <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', gap: '10px', maxWidth: '340px' }}>
            {[t('verificar2fa.panelCheck1'), t('verificar2fa.panelCheck2'), t('verificar2fa.panelCheck3')].map((item) => (
              <div key={item} style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'rgba(219, 234, 254, 0.9)', fontSize: '14px', fontWeight: 600 }}>
                <FaCheckCircle size={14} color="var(--brand-accent)" />
                <span>{item}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ width: '100%', maxWidth: '440px' }}>
            ...
            <button
              ...
              style={{
                ...
                padding: '8px 14px',
                marginBottom: '20px',
                boxShadow: esModoOscuro ? 'none' : '0 8px 18px rgba(var(--brand-primary-rgb), 0.12)',
            ...
            <div style={{
              ...
              <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', marginBottom: '24px' }}>
                <div style={{
                  ...
                  boxShadow: esModoOscuro ? 'none' : '0 12px 28px rgba(var(--brand-primary-rgb),0.16)'
                }}>
                  <FaShieldAlt size={36} color={c.iconColor} />
                </div>
                <h1 style={{ fontSize: '1.55rem', fontWeight: 900, color: esModoOscuro ? '#ffffff' : 'var(--brand-secondary)', margin: '0 0 8px' }}>
                  ...
                  {Array.from({ length: LARGO_CODIGO }).map((_, i) => (
                    <input
                      ...
                      style={{
                        ...
                        boxShadow: codigo[i] ? '0 0 0 4px rgba(var(--brand-primary-rgb), 0.14)' : 'none',
                      }}
                      onFocus={(e) => {
                        if (!error) {
                      ...
                      onBlur={(e) => {
                        ...
                        e.target.style.boxShadow = codigo[i] ? '0 0 0 4px rgba(var(--brand-primary-rgb), 0.14)' : 'none'
                      }}
                    />
                  ))}
                </div>
