function SpecItem({ icon: Icono, label, value }) {
  return (
...
export default function SpecsGrid({ items }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', rowGap: 20, columnGap: 16 }}>
      {items.map((it, i) => <SpecItem key={i} icon={it.Icono} label={it.label} value={it.value} />)}
    </div>
  )
}