    ...
    <div className="mt-8">
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '0 0 16px' }}>
          <FaPlusCircle color={c?.accentText || 'var(--brand-secondary)'} size={14} />
          ...
          {servicios.map((servicio) => {
            ...
            return (
              <button
                ...
                style={{
                  cursor: 'pointer',
                  background: activo ? 'rgba(var(--brand-primary-rgb),0.04)' : 'transparent',
                  borderRadius: 8,
              ...
              >
                <span
                  style={{
                    ...
                    height: 18,
                    borderRadius: 4,
                    border: `1px solid ${activo ? (c?.accentText || 'var(--brand-primary)') : (c?.textSecondary || '#cbd5e1')}`,
                    background: activo ? (c?.accentText || 'var(--brand-primary)') : 'transparent',
                    color: '#fff',
                    flexShrink: 0,
                ...
                <span style={{ color: activo ? (c?.accentText || 'var(--brand-primary)') : (c?.textSecondary || '#94a3b8'), display: 'flex', alignItems: 'center' }}>
                  {getIconForService(servicio.nombre)}
                </span>

                <span style={{ fontSize: 13, fontWeight: activo ? 600 : 500, color: activo ? (c?.accentText || 'var(--brand-primary)') : (c?.textPrimary || '#0f172a') }}>
